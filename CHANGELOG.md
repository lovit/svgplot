...
- 그 한 줄이 하던 일(`numpy.histogram_bin_edges`)을 직접 구현했다. 폴백이 아니라 **대체**다 — 폴백이면 numpy 설치 여부에 따라 같은 입력이 다른 SVG를 내는데, 그건 이 패키지의 정체성("같은 입력 → 같은 SVG")과 어긋난다. 경계는 통상 데이터에서 numpy와 **비트 단위로 동일**하다(9가지 모양 × 12가지 크기 × 전략 7종, 28,000 비교, 불일치 0). 의도적으로 갈리는 곳이 셋 있다: `_MAX_BINS`를 넘는 전략 결과는 거부하고(numpy는 만든다), `stone` 전략은 구현하지 않았으며(**breaking** — 쓰던 호출은 남은 7종을 알려주는 `ValueError`가 된다), **값의 크기가 퍼짐을 압도하는 컬럼**에서 `scott`·`doane` 만 bin 이 하나 둘 갈린다. 둘 다 표준편차를 거치는데 그 크기에서는 `값 − 평균`이 마지막 몇 자리만 남기고 상쇄되고, 그 나머지를 이쪽은 정확히·numpy 는 pairwise 로 더해 반올림이 갈린다. 실측: 크기 1e14 까지 0.00%, 1e15 에서 0.24%, 1e16 에서 1.47%, 최악 두 bin. 초안 둘이 이 문단을 반대 방향으로 틀렸다 — 처음에는 subnormal·float 최대 근처를 지목했으나 실측 0.00% 였고, 다음에는 `math.fsum` 을 지목했으나 CPython 의 `sum` 도 보정을 하므로 둘이 갈리는 입력을 찾지 못했다. 그 초안의 큰 수치들의 진짜 원인은 **분위수 보간**이었다: 가중형이 큰 두 이웃 값의 공통부분을 잃어, 1e15 의 스파이크 컬럼에서 사분위 범위가 0 으로 측정되고 `fd` 가 numpy 의 431 막대 자리에 한 막대를 그렸다. 문서로 적는 대신 `stats/quantile.py` 에서 고쳤고(차이가 표현 가능하면 numpy 와 같은 형태를, 표현 불가능하면 오버플로를 피하는 가중형을 쓴다), 그래서 위 수치가 그만큼 작다. 정수 리스트는 numpy 의 정수 dtype 폭 하한(`width < 1 → 1`)까지 재현하며 bool 컬럼도 같다(numpy 가 `uint8` 로 캐스팅하므로). 대조는 `numpy-parity` extra 를 설치하면 재실행된다.
