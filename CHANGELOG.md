...
- `lineplot`이 `datetime.date` 컬럼을 거부하던 문제. `datetime`이 `date`의 서브클래스라 `isinstance(v, datetime)` 검사가 평범한 `date`를 전부 놓쳤고, 컬럼이 수치 경로로 흘러 `float()`에서 죽었다 — CSV·pandas가 가장 흔히 내놓는 타입이다. `date`는 자정으로 승격되며, 한 컬럼에 `date`와 `datetime`이 섞여도 승격 후 그린다(무손실이고 실제로 나오는 형태다). 축에 위치가 없는 타입(`time`, 문자열)과 날짜에 숫자가 섞인 컬럼은 **컬럼명을 지목하는** `ValueError`로 거부한다.
- 시간축 눈금 라벨이 중복되던 문제. 포매터가 `"%Y-%m-%d"`로 고정돼 있어 3시간 도메인의 눈금 5개가 전부 `2024-01-01`이었다 — 위치는 서로 다른데 이름이 같으니 축이 스스로를 반박한다. 이제 라벨 해상도를 **눈금이 서로 구별되는 가장 거친 포맷**으로 고른다(하루 안이면 `시:분`, 며칠이면 날짜, 몇 달이면 `연-월`, 몇 년이면 연). 눈금 위치도 달력 경계에 맞춘다 — 타임스탬프를 그냥 숫자로 보고 "좋은 수"를 고르면 50,000초 같은 간격이 나와 04:13·18:06에 눈금이 서고, 그래서 날짜만으로는 애초에 구별이 불가능했다. 월 단위는 30일 고정이 아니라 달 필드로 넘긴다(30일 고정은 표류해 매달 다른 날에 눈금이 선다).
