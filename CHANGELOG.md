...
- `lineplot`이 `datetime.date` 컬럼을 거부하던 문제. `datetime`이 `date`의 서브클래스라 `isinstance(v, datetime)` 검사가 평범한 `date`를 전부 놓쳤고, 컬럼이 수치 경로로 흘러 `float()`에서 죽었다 — CSV·pandas가 가장 흔히 내놓는 타입이다. `date`는 자정으로 승격되며, 한 컬럼에 `date`와 `datetime`이 섞여도 승격 후 그린다(무손실이고 실제로 나오는 형태다). 축에 위치가 없는 타입(`time`, 문자열), 날짜에 숫자가 섞인 컬럼, 그리고 `timestamp()`가 처리하지 못하는 `datetime.max` 근처 값은 **컬럼명을 지목하는** `ValueError`로 거부한다.
