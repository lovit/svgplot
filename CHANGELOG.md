...
- 범례 행이 캔버스 아래로 넘치면 **거부한다**(예전에는 캔버스 밖에 그렸다). `heatmap`의 9단계 범례는 상단 여백 아래로 180px, 즉 캔버스 높이 210px을 요구해 최소 높이에서 들어가지 않는다. 가드는 범례가 **점유를 주장하는** 공간이 아니라 실제 **잉크**를 잰다 — 앞의 것으로 재면 들어가는 범례를 거부한다. 실측: `origin/main` 에서 800×600 범례 29개는 마지막 잉크가 y=594 로 정상 렌더되고, 30개가 y=614 로 넘친다. 그 경계 그대로다.
