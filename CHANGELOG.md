...
- `histplot` 패널은 bin 의 **범위와 개수를 모두** 공유한다. 범위만으로는 부족한데, `bins="auto"` 같은 전략은 폭을 각 패널 자신의 값에서 구한 뒤 그 폭으로 범위를 덮기 때문이다 — 눈금이 완전히 일치하는 축 아래에서 15개(5.07px)와 14개(4.79px)가 나온다(실측). 공유되는 개수는 패널들이 고른 것 중 가장 세밀한 쪽이고, `bins=`를 정수로 지정하면 그 값이 그대로 쓰인다.
