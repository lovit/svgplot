...
- 긴 범례 라벨이 캔버스 밖으로 나가 사라지던 문제. 기본 캔버스에서 범례에 남는 폭은 118px 이고 이는 CJK 약 11자다 — 한국어 우선 패키지에서 흔한 길이다. 이제 폭을 추정해 줄이고 전체 문구를 `<title>`에 남긴다. 추정은 글리프를 측정하지 않으므로 근사이며, 그 한계와 어느 방향으로 틀리는지는 `charts/_textwidth.py` 모듈 docstring에 적었다. `treemap`의 타일 라벨도 같은 처리를 받는다. 추정이 상한임이 실측으로 확인된 문자는 출력 가능 ASCII·라틴-1 보충·라틴 확장-A(Arial/Helvetica 335자 전수 검증) + 전각뿐이다. 그 밖(키릴·그리스·아랍·히브리·데바나가리·태국어)은 **예산 대비 비율과 무관하게 항상** 전체 문구를 `<title>`에 남긴다 — 임계값으로는 묶을 수 없기 때문이다. `⸻`(U+2E3B)는 폴백 폰트에서 2.493 em 이라 6자가 예산의 3분의 1로 추정되고도 캔버스를 46px 넘어간다. 유럽어(`São Paulo`, `Bénéfice`, `İstanbul`)가 측정 범위에 든 것은 이 규칙의 비용을 줄이기 위해서다 — 그렇지 않았다면 이런 라벨이 전부 `<title>`을 달았다. 그래도 비용은 있다: 현실적 다국어 라벨 110개 중 57개(51.8%)가 `<title>`을 달고, 그중 46개는 캔버스에 여유가 충분한데도 달린다(키릴·그리스·아랍·히브리·태국·데바나가리는 글자 수와 무관하게 100%). `main`에서는 `<title>`이 0건이었고 대신 4개가 복구 불가능하게 캔버스를 벗어났다.
