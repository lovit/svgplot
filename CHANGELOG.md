...
- 그 한 줄이 하던 일(`numpy.histogram_bin_edges`)을 직접 구현했다. 폴백이 아니라 **대체**다 — 폴백이면 numpy 설치 여부에 따라 같은 입력이 다른 SVG를 내는데, 그건 이 패키지의 정체성("같은 입력 → 같은 SVG")과 어긋난다. 경계는 통상 데이터에서 numpy와 **비트 단위로 동일**하다(9가지 모양 × 12가지 크기 × 전략 7종, 28,000 비교, 불일치 0). 의도적으로 갈리는 곳이 셋 있다: `_MAX_BINS`를 넘는 전략 결과는 거부하고(numpy는 만든다), `stone` 전략은 구현하지 않았으며(**breaking** — 쓰던 호출은 남은 7종을 알려주는 `ValueError`가 된다), **값의 크기가 퍼짐을 압도하는 컬럼**(에포크 나노초 타임스탬프, `2**53` 근처의 ID 컬럼처럼 실재하는 크기다)에서 bin 하나가 차이난다 — float 격자가 모자라서가 아니라 이쪽이 **정확히** 더하기 때문이다(`math.fsum` vs `numpy.std` 의 pairwise 합, 그리고 분위수 보간식 차이). 실측 불일치율은 크기/퍼짐 비가 1e12 까지 0.00%, 1e13 에서 0.24%, 그 이상에서 3.2~4.5%. 초안이 지목했던 subnormal(0.09%)·float 최대 근처(0.00%)는 실측상 원인이 아니었다. 정수 리스트 입력은 numpy 의 정수 dtype 폭 하한(`width < 1 → 1`)까지 재현하므로 일치한다. 대조는 `numpy-parity` extra 를 설치하면 재실행된다.
