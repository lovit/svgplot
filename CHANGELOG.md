...
- `histplot` 패널은 bin 의 **범위와 폭을 모두** 공유한다. 범위만으로는 부족한데, `bins="auto"` 같은 전략은 폭을 각 패널 자신의 값에서 구한 뒤 그 폭으로 범위를 덮기 때문이다 — 눈금이 완전히 일치하는 축 아래에서 두 패널이 폭 0.600000 과 0.200000 을 고른다(실측) — 막대 하나가 뜻하는 양이 3배 차이난다. 공유되는 것은 개수가 아니라 **폭**이다: 개수는 그것을 고른 범위를 떠나면 의미가 없어서, 1..3 과 100..102 에 각각 막대 3개인 두 패널이 고른 3 을 1..102 에 다시 펴면 패널마다 통막대 하나가 된다. `bins=`를 정수로 지정하면 그대로 존중한다 — 범위가 공유되면 모든 패널이 같은 구간을 같은 수로 나누므로 경계가 저절로 맞고, 공유할 것이 없다.
