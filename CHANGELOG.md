...
- **요약 상한**: 이름은 최대 6개, 합쳐서 60자까지 나열하고 나머지는 `and N more`로 센다. 개수 상한만으로는 한 이름이 200자인 경우를 막지 못하고, 글자 수 상한만으로는 두 글자짜리 이름 열두 개를 막지 못해 **둘 다** 둔다. 이름을 잘라 맞추는 일은 없다 — 잘린 이름은 다른 이름이다. 나열은 항상 앞에서부터의 **접두**이며, 첫 이름조차 한도를 넘으면 괄호 전체를 생략하고 개수만 남긴다. 테스트가 고정한 16종의 문장은 35–102자(중앙값 61.5)이고, 그 수치는 테스트가 표에서 다시 계산하므로 문서와 어긋날 수 없다. 상한이 묶는 것은 **이름 부분**뿐이고(묶음당 60자, `heatmap`만 열·행 두 묶음) 나머지 절은 개수와 값의 **자릿수**로만 자라므로 데이터가 1000배가 돼도 문장은 몇 자 길어질 뿐이다. 이름 길이 1–100 × 범주 수 7–12,345로 탐색한 `heatmap` 최장 문장이 264자이며 이 천장도 테스트가 잡는다.
