...
- 시간축 눈금 라벨이 중복되던 문제. 포매터가 `"%Y-%m-%d"`로 고정돼 있어 3시간 도메인의 눈금이 전부 `2024-01-01`이었다(개수는 시작 시각에 따라 5~6개) — 위치는 서로 다른데 이름이 같으니 축이 스스로를 반박한다. 이제 라벨 해상도를 **눈금이 서로 구별되는 가장 거친 포맷**으로 고른다(하루 안이면 `시:분`, 며칠이면 날짜, 몇 달이면 `연-월`, 몇 년이면 연). 눈금 위치도 달력 경계에 맞춘다 — 타임스탬프를 그냥 숫자로 보고 "좋은 수"를 고르면 50,000초 같은 간격이 나와 04:13·18:06에 눈금이 서고, 그래서 날짜만으로는 애초에 구별이 불가능했다. 월 단위는 30일 고정이 아니라 달 필드로 넘긴다(30일 고정은 표류해 매달 다른 날에 눈금이 선다). 두 주 이하의 고정 간격은 첫날 자정 기준으로 정렬되므로 이후 달 경계와는 어긋날 수 있다 — 그래서 두 주를 넘으면 달 필드로 넘긴다.
