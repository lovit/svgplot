...
def test_a_page_rule_outranks_the_chart_rule_it_has_to_override() -> None:
    ...
    **The comparison alone is a weak tripwire, so the margin is asserted too.** Mutated one at
    a time, and transcribed rather than summarised -- three earlier versions of this paragraph
    each stated a summary that did not survive being run:

    ========================================  ============  ==================  ==============
    mutation                                  page rule     comparison          id assertion
    ========================================  ============  ==================  ==============
    ``#id`` written as ``.id``                ``(0,3,1)``   still wins          **red**
    ``#id`` written as a bare element         ``(0,2,2)``   still wins          **red**
    selector wrapped in ``:where()``          ``(0,1,1)``   still wins          **red**
    targets ``:where()`` not ``:is()``        ``(1,1,1)``   still wins          passes
    ``scope.py`` ``:where()`` -> ``:is()``    unchanged     still wins [1]_     passes
    both ends in ``:where()``                 ``(0,0,1)``   **red**             **red**
    ========================================  ============  ==================  ==============

    .. [1] the chart side goes ``(0,1,0)`` -> ``(0,2,0)``; ``(1,2,1)`` beats it either way, so
       the toggles keep working and this test is right not to fire. The gallery byte-diff
       catches that one.

    So the comparison is kept for what it is -- the invariant, re-derived from real output
    rather than from a number written down once -- and the id assertion carries the weight for
    every mutation that touches the page rule's own strength. The two it does not fire on are
    the two that leave the id in place, which is correct.
    """
    example = _stub([("bars", _BAR)], {1: "toggle"}).examples[0]
    (controls,) = example.controls
    rules = interaction.css(controls)
