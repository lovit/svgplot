...
sp.heatmap(DIFF, x="시간", y="요일", values="증감", cmap="coolwarm", center=0.0, annot=True)""",
    ...
    (
        "tooltip=True — 셀마다 자기 행·열·값·단계를 말한다",
...
NOTES = [
    ...
    "단계 번호도 함께 말한다. 범례가 그 번호로 매겨져 있어서, 셀을 옆의 색 눈금과 견주는 독자가 묻는 것이 "
    '"이게 아홉 중 몇 번째냐" 이기 때문이다.',
    "셀 하나에 <title> 하나이므로 격자가 크면 파일이 그만큼 커진다. tooltip=False 가 기본인 이유다 — "
    "200x200 격자는 3,385KB 에서 6,346KB 가 된다. 크기 경고도 이 값을 함께 센다.",
