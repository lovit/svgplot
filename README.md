...
- **`estimator=`** — `lineplot` · `barplot` · `areaplot` · `radarplot`. 같은 x(레이더는 같은 스포크)에 여러 행이 있을 때 `"mean"`/`"sum"`/`"median"` 등으로 접는다. 지정하지 않으면 차트별 기본 규칙이 적용되고, 그 규칙이 행을 버리는 둘 — `barplot` 과 `radarplot` — 은 `AggregationWarning` 으로 몇 행이 남았는지 말해준다(`areaplot` 은 합산하고 `lineplot` 은 둘 다 그리므로 잃는 것이 없어 경고하지 않는다)
